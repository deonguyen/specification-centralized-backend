...
class TestExecutionTraceModel(models.Model):
    """Represents a specification created by a user."""

    milestone = models.ForeignKey(
        ProjectMilestoneModel,
        related_name="test_execution_trace_milestone_id_x_project_milestone_id",
    ...
    project = models.ForeignKey(
        ProjectModel,
        related_name="test_execution_trace_project_id_x_project_id",
        on_delete=models.DO_NOTHING,
        db_column="project_id",
    )
    project_specification = models.ForeignKey(
        ProjectSpecificationModel,
        related_name="test_execution_trace_project_specification_id_x_project_specification_id",
        on_delete=models.DO_NOTHING,
        db_column="project_specification_id",
    )
    specification = models.ForeignKey(
        SpecificationModel,
        related_name="test_execution_trace_specification_id_x_specification_id",
        on_delete=models.DO_NOTHING,
        db_column="specification_id",
    )
    specification_revision = models.ForeignKey(
        SpecificationRevisionModel,
        related_name="test_execution_trace_specification_revision_id_x_specification_revision_id",
        on_delete=models.DO_NOTHING,
        db_column="specification_revision_id",
    )
    test_execution_status = models.CharField(
        max_length=255,
        null=True,
        blank=True,
    )
